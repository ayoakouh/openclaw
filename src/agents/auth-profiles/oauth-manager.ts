...
export function createOAuthManager(adapter: OAuthManagerAdapter) {
    ...
    const updated = await updateAuthProfileStoreWithLock({
      ...
      updater: (store) => {
        ...
        if (existing?.type === "oauth") {
          validateSettlementCredential(params.validateCredential, existing);
        }
  ...
  async function settlePeerClaimsUnderRefreshLock(params: {
    ...
    if (authoritativeSharedCredential) {
      validateSettlementCredential(params.validateCredential, authoritativeSharedCredential);
    }
    settleOAuthRefreshPeerClaims({
      profileId: params.claim.profileId,
